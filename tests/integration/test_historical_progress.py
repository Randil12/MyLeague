...
def test_empty_history_is_archived_and_checkpointed(infrastructure, monkeypatch, tmp_path):
    ...
    paused_players = []
    try:
        # The CI database is shared: isolate eligibility, not the collector SQL.
        # Otherwise max_players=1 may select a roster left by an earlier test.
        with conn, conn.cursor() as cur:
            cur.execute("""UPDATE audit.riot_tracked_players SET is_tracked=false
                WHERE is_tracked AND puuid<>%s RETURNING puuid""", (player,))
            paused_players = [row[0] for row in cur.fetchall()]
        ...
        assert api.call_args.args[0] == player
        objects = s3.list_objects_v2(Bucket='ci-live',Prefix=f'bronze/riot/region=euw1/historical_match_ids/puuid={player}/')
        assert objects['KeyCount'] == 1
    finally:
        with conn, conn.cursor() as cur:
            cur.execute('UPDATE audit.riot_tracked_players SET is_tracked=true WHERE puuid=ANY(%s)',
                        (paused_players,))
