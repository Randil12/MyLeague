...
import {liveDuration} from './liveDuration.mjs';
...
export default function LiveGames({revision}:{revision:number}) {
  ...
  return <div className="live-page"><LiveRoster revision={revision+tick} onChange={()=>setTick(v=>v+1)}/><aside className="note"><Radio size={20}/><div><strong>Observations spectator · actualisation de l’écran toutes les 15 secondes</strong><p>Suivi des joueurs ci-dessus, indépendant des filtres des autres analyses.</p></div></aside>
    ...
    {service.loading||players.loading?<div className="loading" role="status"><RefreshCw size={18}/> Actualisation des observations…</div>:<>
      ...
      <div className="live-grid">{players.rows.map((p,index)=>{
        ...
        return <article className={`panel live-card ${stale?'stale':''}`} key={`${p.player_name}-${index}`}><div className="panel-head"><div><span className="eyebrow">JOUEUR SUIVI · EUW</span><h2>{String(p.player_name)}</h2></div><span className={`badge ${stale||p.status==='error'?'warning':''}`}>{stale?'Observation périmée':statusLabels[String(p.status)]||'État inconnu'}</span></div>
          {active&&<><div className="live-match"><span>Partie #{String(p.game_id)}</span><span>Début déclaré : {when(p.game_started_at)}</span><span title="Temps écoulé depuis le début déclaré par Riot jusqu’à la dernière observation. Ce n’est pas l’horloge exacte du jeu.">Durée estimée à l’observation : <strong>{liveDuration(p.game_started_at,p.checked_at)}</strong>{stale?' · observation périmée':''}</span></div><div className="live-teams"><div><h3>Côté bleu</h3><Composition value={String(p.blue_team||'Composition indisponible')}/></div><div><h3>Côté rouge</h3><Composition value={String(p.red_team||'Composition indisponible')}/></div></div></>}
          {!active&&<p className="live-message">{p.status==='error'?`Échec de l’observation (${String(p.error_code||'erreur inconnue')}). Aucune conclusion sur la partie actuelle.`:p.status==='other_queue'?'Une partie a été détectée hors classé solo ; elle n’est pas affichée dans ce périmètre.':'Aucune partie active renvoyée par Riot lors de cette vérification.'}</p>}
