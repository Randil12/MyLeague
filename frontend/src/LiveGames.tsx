...
export default function LiveGames({revision}:{revision:number}) {
  ...
  return <div className="live-page"><LiveRoster revision={revision+tick} onChange={()=>setTick(v=>v+1)}/><aside className="note"><Radio size={20}/><div><strong>Observations spectator · actualisation de l’écran toutes les 15 secondes</strong><p>Cette vue ne dépend ni du patch ni du joueur sélectionné dans les autres analyses. Le service interroge uniquement la liste de joueurs définie ci-dessus. Ce n’est pas un flux continu de positions, d’or ou de kills.</p></div></aside>
