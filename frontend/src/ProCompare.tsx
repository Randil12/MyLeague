...
function Comparison({year,revision}:{year:number;revision:number}) {
  ...
  return <>
    <aside className="note"><div><strong>Comparer à périmètre constant</strong><p>Filtres appliqués à la liste et aux statistiques. Région du tournoi ; couverture Leaguepedia partielle.</p></div></aside>
    ...
    {!valid?<p className="empty">Choisis de deux à cinq joueurs distincts pour afficher la comparaison.</p>:<>
      ...
      {!comparison.loading&&!comparison.error&&<section className="panel pro-comparison"><div className="panel-head"><h2>Comparaison individuelle</h2><span className="badge">{year} · Compétition</span></div>
        {selected.some(r=>!r||Number(r.games)<5)&&<p className="note">Échantillon inférieur à cinq parties pour au moins un joueur.</p>}
        ...
        <p className="panel-foot">Gold@15 indisponible dans cette source.</p>
      </section>}
