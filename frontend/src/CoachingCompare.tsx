...
function PlayerCard({year,revision}:{year:number;revision:number}) {
  ...
  return <section className="panel coaching-player"><div className="filters">
    ...
    {source==='pro'&&valid&&<aside className="note"><div><strong>Comptes soloQ déclarés sur Leaguepedia</strong>{accounts.loading?<p>Lecture du profil…</p>:accounts.error?<p role="alert">{accounts.error}</p>:<p className="rune-source">{String(accounts.rows[0]?.reported_accounts||'Aucun compte renseigné dans Cargo.')}</p>}<p>Comptes déclarés, non vérifiés. Vérifie le Riot ID avant de l’ajouter au roster.</p></div></aside>}
  </section>;
}

export default function CoachingCompare({revision}:{revision:number}) {
  const [year,setYear]=useState(new Date().getFullYear());
  return <><div className="filters"><label className="field">Année (UTC)<input type="number" min={2000} max={2100} value={year} onChange={e=>{const n=Number(e.target.value);if(n>=2000&&n<=2100)setYear(n);}}/></label></div><p className="note">Compare deux joueurs. Sources distinctes : SoloQ et compétition professionnelle. Gold@15 indisponible.</p><div className="split"><PlayerCard key={`a-${year}`} year={year} revision={revision}/><PlayerCard key={`b-${year}`} year={year} revision={revision}/></div></>;
}
