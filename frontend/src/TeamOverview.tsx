...
export default function TeamOverview({patch,roster,revision}:{patch:string;roster:string[];revision:number}) {
  const result=useData('/api/team/summary',{patch,roster},revision,roster.length>=1&&roster.length<=5);
  return <section className="panel"><div className="panel-head"><div><h2>Suivi du groupe</h2><p>Statistiques individuelles SoloQ · {patch?`patch ${patch}`:'tous les patches'}.</p></div></div>
